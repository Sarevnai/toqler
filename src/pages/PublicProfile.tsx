        ...
        <motion.div className={`relative z-10 rounded-2xl pt-5 pb-3 px-6 mx-4 mb-5 ${hasCover ? "mt-3" : "-mt-4"}`} style={{ background: T.card }} variants={slideUp(0.3)} initial="hidden" animate="visible">
          ...
          {showCompanyHeader && company && (
            <div className="flex items-center justify-between gap-4 mt-5 pb-5 mb-0" style={{ borderBottom: `1px solid ${T.cardBorder}` }}>
              {company.logo_url ? <img src={company.logo_url} alt={company.name} className="h-20 opacity-85 shrink-0" /> : <span className="text-sm font-medium shrink-0" style={{ color: T.text2 }}>{company.name}</span>}
